...
- Every agent starts with `GIT_OPTIONAL_LOCKS=0`, so its `git status` no longer takes `.git/index.lock` from under the
  owner's `git commit`, and with `HERDR_REVIEW_AGENT` naming it.
