...
3. Check the report as in Phase 4, by the scope: in scope `commits` the commit, with the subject compared to the first line of `{RUN_DIR}/fix-<i>-commit.txt`, and the one prompt to commit when it is missing; in scope `worktree` only that HEAD did not move, with no prompt to commit. Note the hash, and move to the next issue.
