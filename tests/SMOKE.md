...
8. Dirty tree: on a branch with at least one commit of its own, before launching, edit a tracked file and add an untracked file and an untracked directory unrelated to the change. The summary shows `вне ревью` with both counts, `uncommitted.txt` lists them, no review mentions them, and after the run they are unchanged and in no commit.
...
10. `bin/herdr-review close` refuses while the run is in progress (try it before item 7) and closes every tab of the run once it has finished.
11. MCP refusal: in the repository of item 9, give a claude reviewer profile its own `--settings <file>` without `enableAllProjectMcpServers`: that reviewer turns ` ✗` with "Claude Code asks to approve this project's MCP servers", no key reaches its tab, and `.claude/settings.local.json` stays as it was.
12. Build queue: preset `default` (two reviewers) on a repository whose test suite takes a minute or more. Both reviewers run the tests through `herdr-review exclusive`: while one runs, `herdr-review status` in that repository shows `очередь сборок: занята — <agent> (прогон <id>): …`, and the other reviewer's tab shows `herdr-review exclusive: waiting` or a retry after exit code 75. `runner.log` has their `exclusive:` lines, and no `running` line of one comes between the `running` and `done` lines of the other.
