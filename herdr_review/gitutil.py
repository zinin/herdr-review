...
def commit_hashes(repo: Path | str, range_: str) -> list[str]:
    """The abbreviated hashes of the commits in <range_> along the first parents, newest first: a merge commit that
    brings the base in is listed alone, without the base's commits. A fast-forward to the base still lists them: it
    puts them on the first-parent line. git prints the hash alone, one per line: no subject can split an entry, and
    no colour or signature line joins one even under color.ui=always and log.showSignature=true."""
