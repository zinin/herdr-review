...
- **Builds and tests:** the reviewers share your working tree and your machine, so their heavy commands — builds, tests, dependency installs, servers, containers — take turns: the prompts send each one through `herdr-review exclusive`, one at a time among all reviews on this machine, and a reviewer reads code while it waits. The fixer runs its tests the same way, and commits through the wrapper too, since a commit's hooks may build or test. `herdr-review status` shows who holds the queue (`очередь сборок: занята — …`). `run fail` stops a command of the agent it takes out of the run, and the wrapper runs nothing more for that agent; `run finish` and `close` stop any command of their run that still holds the queue.
- **The queue's limits:** only commands sent through the wrapper take turns, and the agents send theirs there because their prompts say so: an agent that ignores its prompt collides with the others as before. A stop reaches every process under the wrapper and nothing else: not containers started through a daemon (`docker compose up -d`), which a timeout leaves running, and on macOS not a process whose parent has died, such as the background job of an `sh -c` that Ctrl-C killed. A Ctrl-C or a hangup from your terminal reaches the whole process group, the command's processes included, so the wrapper passes it on only to the processes that left the group: a program that takes a second Ctrl-C as "quit now", such as `docker compose up` in the foreground, keeps its graceful stop, and whatever still runs 10 s later gets SIGKILL. A SIGINT or SIGHUP sent to the wrapper alone therefore reaches its command only as that SIGKILL; SIGTERM, which `run fail` and `close` send, the wrapper passes on to everything under it. On Linux a stop also ends the daemons that the command started, such as a Gradle daemon, so the next build starts a new one; a command that ends by itself leaves them running. A wrapper killed with SIGKILL frees the queue at once, while what its command started lives on (on macOS, the command itself too). There is one queue per `runs_dir`: with a relative `runs_dir`, your own calls outside a review queue per directory they run in.
